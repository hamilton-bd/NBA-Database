...
def PlayerShooting(player_name):
    conn = MySQLdb.connect(host='localhost',
                           ...
                           cursorclass=MySQLdb.cursors.DictCursor)
    c = conn.cursor()
    c.execute('SELECT Year, Games, Minutes, TruePer, ThreePer, FTPer, ShotPer'
              ' FROM Player JOIN Stats ON Player.StatsID = Stats.StatsID WHERE'
...
def PlayerTeam(player_name):
    conn = MySQLdb.connect(host='localhost',
                           ...
                           cursorclass=MySQLdb.cursors.DictCursor)
    c = conn.cursor()
    c.execute('SELECT Year, PlayerName, Age, YearsExp, Height, Weight, Salary,'
              ' TeamName FROM Player JOIN Team ON Player.TeamID = Team.TeamID'
...
def json_PlayerShooting(player_name):
    conn = MySQLdb.connect(host='localhost',
                           ...
                           cursorclass=MySQLdb.cursors.DictCursor)
    c = conn.cursor()
    c.execute('SELECT Year, Games, Minutes, TruePer, ThreePer, FTPer,'
              ' ShotPer FROM Player JOIN Stats ON Player.StatsID ='
              ' Stats.StatsID WHERE PlayerName=%s;', [player_name])
    result_list = []
    if c.rowcount > 0:
        rs = c.fetchall()
        result_list = [r for r in rs]
...
def xml_player(player_name):
    conn = MySQLdb.connect(host='localhost',
                           ...
                           cursorclass=MySQLdb.cursors.DictCursor)
    c = conn.cursor()
    c.execute('SELECT PlayerName, Year, Age, YearsExp, Height, Weight, Salary'
              ' FROM Player WHERE PlayerName = %s;', [player_name])
    result_list = []
    if c.rowcount > 0:
        rs = c.fetchall()
        result_list = [r for r in rs]
        ...
        for row in result_list:
            s += '<Player>'
            s += '<PlayerName>{}</PlayerName><Year>{}</Year><Age>{}</Age>' \
                 '<YearsExp>{}</YearsExp><Height>{}</Height><Weight>{}' \
                 '</Weight><Salary>{}</Salary>' \
                 .format(row['PlayerName'], row['Year'], row['Age'],
                         row['YearsExp'], row['Height'], row['Weight'],
                         row['Salary'])
            s += '</Player>'
            s += '</Players>'
        return s
